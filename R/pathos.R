...
#' plot(w)
#' laea <- pathologicule(w, "+proj=laea +lon_0=147 +lat_0=-42")
#' stere <- pathologicule(w, "+proj=stere +lon_0=147 +lat_0=-42")
#' plot(laea, add = TRUE, col = "dodgerblue")
#' plot(stere, add = TRUE, col = "firebrick")
#'
#' p <- spTransform(subset(wrld_simpl, coordinates(wrld_simpl)[,2] < -20), "+proj=stere +lat_0=-90")
#' plot(extent(p) + 1e6, asp = 1, type = "n"); plot(p, add = TRUE)
#' laea <- pathologicule(p, "+proj=laea +lon_0=147 +lat_0=-72")
